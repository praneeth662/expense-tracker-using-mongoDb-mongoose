...
window.addEventListener("DOMContentLoaded", async() => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/user/expense/page/?page=${page}`, {
        headers: {
          "Authorization": token,
...
async function isPremium(){
    try{
        const user = await axios.get("http://localhost:5000/user/status", { headers: {"Authorization" : token }})
        // console.log(user)
        if(user.data === true){
...
async function showLeaderBoard(e){
    ...
    try{
        ...
        const users = await axios.get("http://localhost:5000/premium/leadershipboard", { headers: {"Authorization" : token }});
        // console.log(users);
...
async function showTotalExpense() {
   ...
   try{
   const response = await axios.get("http://localhost:5000/user/expense", { headers: {"Authorization" : token }})
        // console.log(response)
...
async function onSubmit(e) {
    ...
    try{
        const response = await axios.post('http://localhost:5000/user/expense', userExpense,{
             headers: {
                "Authorization" : token 
            }
        });
    ...
    try {
        if(e.target.classList.contains('delete')){
            if(confirm('Are You Sure?')){
                ...
                id = li.id;
                await axios.delete(`http://localhost:5000/user/delete/${id}`, { headers: {"Authorization" : token }})
                expense.removeChild(li);
                showTotalExpense();
            }
            }
   ...
   //Edit item//
    ...
    async function editUser(e) {
        try{
            if(e.target.classList.contains('edit')){
                var li=e.target.parentElement;
                id = li.id;
                const response = await axios.get(`http://localhost:5000/user/edit/${id}`, { headers: {"Authorization" : token }});
                console.log(response)
                expense.removeChild(li);
    ...
    async function updateItem(e){
        ...
        try{
            const response = await axios.put(`http://localhost:5000/user/edit/${id}`, updatedExpense, { headers: {"Authorization" : token }})
            showOnScreen(response.data);
    ...
    async function payment(e) {
        ...
        rzp1.on('payment.failed', async function (response){
            ...
            await axios.post("http://localhost:5000/purchase/transactionfailstatus", response.error.metadata ,{ headers: {"Authorization" : token }})
